<?php
namespace m2i\project\Model\DAO;

use m2i\project\Model\Entity\TrainersSkillDTO;

class TrainersSkillDAO implements ITrainersSkillDAO {

    /**
    * @var \PDO
    */
    private $pdo;

    /**
     * @var \PDOStatement;
     */
    private $selectStatement;


    /**
    * DAOClient constructor.
    * @param \PDO $pdo
    */
    public function __construct(\PDO $pdo)
    {
    $this->pdo = $pdo;
    }

    /**
    * @return $this
    */
    public function findAll(){
        $sql = "SELECT * FROM trainers_skills";
        $this->selectStatement = $this->pdo->query($sql);
        return $this;
    }

    /**
    * @param array $pk
    * @return $this
    */
    public function findOneById(array $pk){
        $sql = "SELECT * FROM trainers_skills WHERE trainer_id=?  AND skill_id=? ";
        $statement = $this->pdo->prepare($sql);
        $statement->execute($pk);
        $this->selectStatement = $statement;
        return $this;
    }

    /**
    * @param array $search
    * @return $this
    */
    public function find(array $search = [], array $orderBy = [], array $limit = []){
        $sql = "SELECT * FROM trainers_skills ";

        if(count($search)>0){
            $sql .= " WHERE ";
            $cols = array_map(
                function($item){
                    return "$item=:$item";
                }, array_keys($search)
            );

            $sql .= implode(" AND ", $cols);
        }

        if(count($orderBy)>0){
            $sql .= "ORDER BY ";
            $cols = array_map(
                function($item) use($orderBy){
                    return "$item ". $orderBy[$item];
                },
                array_keys($orderBy)
            );
            $sql .= implode(", ", $cols);
        }

        if(count($limit) >0){
            $sql .= " LIMIT ".$limit[0];
            if(isset($limit[1])){
                $sql .= " OFFSET ". $limit[1];
            }
        }

        $statement = $this->pdo->prepare($sql);
        $statement->execute($search);
        $this->selectStatement = $statement;
        return $this;
    }


    /**
    * @return array
    */
    public function getAllAsArray(){
        return $this->selectStatement->fetchAll(\PDO::FETCH_ASSOC);
    }

    /**
    * @return array
    */
    public function getAllAsArrayGroupedById(){
        $data =  $this->selectStatement->fetchAll(\PDO::FETCH_ASSOC|\PDO::FETCH_GROUP);

        $data = array_map(
            function ($item){
                return $item[0];
            }
            , $data
        );
        
        if($data){
            return $data;
        } else {
            throw new Exception("Ancun résultat pour cette requête");
        }
        
    }

    /**
    * @return array
    */
    public function getOneAsArray(){
        $data = $this->selectStatement->fetch(\PDO::FETCH_ASSOC);

        if($data){
            return $data;
        } else {
            throw new Exception("Ancun résultat pour cette requête");
        }
    }

    /**
    * @return array
    */
    public function getAllAsEntity(){
        $this->selectStatement->setFetchMode(\PDO::FETCH_CLASS, TrainersSkillDTO::class);
        $data = $this->selectStatement->fetchAll();

        if($data){
            return $data;
        } else {
            throw new Exception("Ancun résultat pour cette requête");
        }
    }

    /**
    * @return TrainersSkillDTO
    */
    public function getOneAsEntity(){
        $this->selectStatement->setFetchMode(\PDO::FETCH_CLASS, TrainersSkillDTO::class);
        $data = $this->selectStatement->fetch();

        if($data){
            return $data;
        } else {
            throw new Exception("Ancun résultat pour cette requête");
        }
    }

    /**
    * @param TrainersSkillDTO $trainersSkill
    */
    public function save(TrainersSkillDTO $trainersSkill){
        if($trainersSkill->getId() == null){
            $pk = $this->insert($trainersSkill);
            $trainersSkill->setId($pk);
        } else {
            $this->update($trainersSkill);
        }
    }

    /**
    * @param TrainersSkillDTO $trainersSkill
    * @return int
    */
    private function insert(TrainersSkillDTO $trainersSkill){
        $sql = "INSERT INTO trainers_skills (trainer_id, skill_id) VALUES ( ?, ? )";
        $statement = $this->pdo->prepare($sql);
        $statement->execute([
            $trainersSkill->getId(), $trainersSkill->getId()
        ]);
        return $this->pdo->lastInsertId();
    }

    /**
    * @param TrainersSkillDTO $trainersSkill
    */
    private function update(TrainersSkillDTO $trainersSkill){
        $sql = "UPDATE trainers_skills SET trainer_id=? , skill_id=?  WHERE trainer_id=?  AND skill_id=? ";
        $data = array(
            $trainersSkill->getId(),
$trainersSkill->getId(),
$trainersSkill->getId(),
$trainersSkill->getId()
        );
        $statement = $this->pdo->prepare($sql);

        $statement->execute([
            $trainersSkill->getId(),
$trainersSkill->getId(),
$trainersSkill->getId(),
$trainersSkill->getId()

        ]);

    }

    /**
    * @param TrainersSkillDTO $trainersSkill
    * @return bool
    */
    public function delete(TrainersSkillDTO $trainersSkill){
        if($trainersSkill->getId() != null){
            $sql = "DELETE FROM trainers_skills WHERE trainer_id=?  AND skill_id=? ";
            $statement = $this->pdo->prepare($sql);
            return $statement->execute([$trainersSkill->getId(), 
$trainersSkill->getId()]);
        }
    }

}