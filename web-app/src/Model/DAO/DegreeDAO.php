<?php
namespace m2i\project\Model\DAO;

use m2i\project\Model\Entity\DegreeDTO;

class DegreeDAO implements IDegreeDAO {

    /**
    * @var \PDO
    */
    private $pdo;

    /**
     * @var \PDOStatement;
     */
    private $selectStatement;


    /**
    * DAOClient constructor.
    * @param \PDO $pdo
    */
    public function __construct(\PDO $pdo)
    {
    $this->pdo = $pdo;
    }

    /**
    * @return $this
    */
    public function findAll(){
        $sql = "SELECT * FROM degree";
        $this->selectStatement = $this->pdo->query($sql);
        return $this;
    }

    /**
    * @param array $pk
    * @return $this
    */
    public function findOneById(array $pk){
        $sql = "SELECT * FROM degree WHERE id=? ";
        $statement = $this->pdo->prepare($sql);
        $statement->execute($pk);
        $this->selectStatement = $statement;
        return $this;
    }

    /**
    * @param array $search
    * @return $this
    */
    public function find(array $search = [], array $orderBy = [], array $limit = []){
        $sql = "SELECT * FROM degree ";

        if(count($search)>0){
            $sql .= " WHERE ";
            $cols = array_map(
                function($item){
                    return "$item=:$item";
                }, array_keys($search)
            );

            $sql .= implode(" AND ", $cols);
        }

        if(count($orderBy)>0){
            $sql .= "ORDER BY ";
            $cols = array_map(
                function($item) use($orderBy){
                    return "$item ". $orderBy[$item];
                },
                array_keys($orderBy)
            );
            $sql .= implode(", ", $cols);
        }

        if(count($limit) >0){
            $sql .= " LIMIT ".$limit[0];
            if(isset($limit[1])){
                $sql .= " OFFSET ". $limit[1];
            }
        }

        $statement = $this->pdo->prepare($sql);
        $statement->execute($search);
        $this->selectStatement = $statement;
        return $this;
    }


    /**
    * @return array
    */
    public function getAllAsArray(){
        return $this->selectStatement->fetchAll(\PDO::FETCH_ASSOC);
    }

    /**
    * @return array
    */
    public function getAllAsArrayGroupedById(){
        $data =  $this->selectStatement->fetchAll(\PDO::FETCH_ASSOC|\PDO::FETCH_GROUP);

        $data = array_map(
            function ($item){
                return $item[0];
            }
            , $data
        );
        
        if($data){
            return $data;
        } else {
            throw new Exception("Ancun résultat pour cette requête");
        }
        
    }

    /**
    * @return array
    */
    public function getOneAsArray(){
        $data = $this->selectStatement->fetch(\PDO::FETCH_ASSOC);

        if($data){
            return $data;
        } else {
            throw new Exception("Ancun résultat pour cette requête");
        }
    }

    /**
    * @return array
    */
    public function getAllAsEntity(){
        $this->selectStatement->setFetchMode(\PDO::FETCH_CLASS, DegreeDTO::class);
        $data = $this->selectStatement->fetchAll();

        if($data){
            return $data;
        } else {
            throw new Exception("Ancun résultat pour cette requête");
        }
    }

    /**
    * @return DegreeDTO
    */
    public function getOneAsEntity(){
        $this->selectStatement->setFetchMode(\PDO::FETCH_CLASS, DegreeDTO::class);
        $data = $this->selectStatement->fetch();

        if($data){
            return $data;
        } else {
            throw new Exception("Ancun résultat pour cette requête");
        }
    }

    /**
    * @param DegreeDTO $degree
    */
    public function save(DegreeDTO $degree){
        if($degree->getId() == null){
            $pk = $this->insert($degree);
            $degree->setId($pk);
        } else {
            $this->update($degree);
        }
    }

    /**
    * @param DegreeDTO $degree
    * @return int
    */
    private function insert(DegreeDTO $degree){
        $sql = "INSERT INTO degree (degree_label, degree_description, degree_level_id) VALUES ( ?, ?, ? )";
        $statement = $this->pdo->prepare($sql);
        $statement->execute([
            $degree->getDegreeLabel(), $degree->getDegreeDescription(), $degree->getDegreeLevelId()
        ]);
        return $this->pdo->lastInsertId();
    }

    /**
    * @param DegreeDTO $degree
    */
    private function update(DegreeDTO $degree){
        $sql = "UPDATE degree SET degree_label=? , degree_description=? , degree_level_id=?  WHERE id=? ";
        $data = array(
            $degree->getDegreeLabel(),
$degree->getDegreeDescription(),
$degree->getDegreeLevelId(),
$degree->getId()
        );
        $statement = $this->pdo->prepare($sql);

        $statement->execute([
            $degree->getDegreeLabel(),
$degree->getDegreeDescription(),
$degree->getDegreeLevelId(),
$degree->getId()

        ]);

    }

    /**
    * @param DegreeDTO $degree
    * @return bool
    */
    public function delete(DegreeDTO $degree){
        if($degree->getId() != null){
            $sql = "DELETE FROM degree WHERE id=? ";
            $statement = $this->pdo->prepare($sql);
            return $statement->execute([$degree->getId()]);
        }
    }

}