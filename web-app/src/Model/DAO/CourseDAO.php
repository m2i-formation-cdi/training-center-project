...
class CourseDAO implements ICourseDAO {
    ...
    */
    public function getAllAsEntity(){
        $this->selectStatement->setFetchMode(\PDO::FETCH_CLASS, CourseDTO::class);
    ...
    * @return int
    */
    private function insert(CourseDTO $course){
        $sql = "INSERT INTO courses (course_label, course_description) VALUES ( ?, ? )";
        $statement = $this->pdo->prepare($sql);
        $statement->execute([
            $course->getCourseLabel(), $course->getCourseDescription()
        ]);
    ...
    * @param CourseDTO $course
    */
    private function update(CourseDTO $course){
        $sql = "UPDATE courses SET course_label=? , course_description=?  WHERE id=? ";
        $data = array(
    ...
    */
    public function delete(CourseDTO $course){
        if($course->getId() != null){
            ...
            $statement = $this->pdo->prepare($sql);
            return $statement->execute([$course->getId()]);
        }
    }
